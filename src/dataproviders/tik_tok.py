...
def get_caption_infos(video_metadata: dict) -> list | None:
    caption_infos = (
...
def extract_caption_from_infos(caption_infos: list):
    if caption_infos:
...
def get_video_metadata_from_link(link: str) -> Any | None:
    try:
...
def process_video(video: dict, liked_links: set) -> Video | None:
    ...
    try:
        ...
        video_metadata = get_video_metadata_from_link(video['Link'])
        ...
        caption_infos = get_caption_infos(video_metadata)
        caption_text = extract_caption_from_infos(caption_infos)
...
def collect_video_from_user_metadata_parallel(user_data: dict, batch_size, workers):
    batch = []
    history_video_list = user_data.get("Your Activity", {}).get("Watch History", {}).get("VideoList", [])
    user_email = user_data.get("Profile", {}).get("Profile Info", {}).get("ProfileMap", {}).get("emailAddress", None)
    user_nickname = user_data.get("Profile", {}).get("Profile Info", {}).get("ProfileMap", {}).get("userName", None)
    logger.info(f"Processing for user email={user_email}, name={user_nickname}")
    logger.info(f"User history contains {len(history_video_list)} videos")
    liked_videos = user_data.get("Your Activity", {}).get("Like List", {}).get("VideoList", [])
    liked_links = {item["Link"] for item in liked_videos}

    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(process_video, video, liked_links): video for video in history_video_list
        }
        for idx, future in enumerate(as_completed(futures), start=1):
            video = futures[future]
            logger.debug(f"[{idx}/{len(history_video_list)}] Completed {video['Link']}")
            try:
...
def process_user_data_info(user_data_path_arch: str):
    with zipfile.ZipFile(user_data_path_arch, 'r') as zip_ref:
        ...
        with open(extracted_file, 'r', encoding='utf-8') as data_file:
            ...
            collect_video_from_user_metadata_parallel(user_data, VIDEO_BATCH_SIZE, VIDEO_WORKER_COUNT)
